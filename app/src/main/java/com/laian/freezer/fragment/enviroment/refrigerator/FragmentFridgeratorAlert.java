...
import com.laian.freezer.adapter.RecycleRefrigertorManageAdapter;
...
public class FragmentFridgeratorAlert extends FragmentAlert {

    String action_getData = className + API.getFridgeatorAlart;
    ...
    public void requestData(String start, String end, String deviceId) {
        showProgressDialog();
        HttpGetController.getInstance().getFridgeratorAlart(deviceId, start, end, className);
    }
     ...
     */
    public static class FragmentFridgeatorManage extends FragmentAlert implements RecycleRefrigertorManageAdapter.OnItemClickListner,SwipeRefreshLayout.OnRefreshListener{
        String action_getData = className + API.getFridgeratorManage;
        String action_add = className + API.addReFridgeator;
        String action_edt = className + API.edtFridgetor;
        String action_del = className + API.delFridgerator;
        String action_getIP = className + API.getTempIP;
        String action_getLocation = className + API.getTemplocations;
        RefrigeratorBean Temps = new RefrigeratorBean();
        ArrayList<RefrigeratorBean.EhmListBean> ehmList=new ArrayList<>();
        ArrayList<Location> locations = new ArrayList<>();
        ArrayList<Ip> ips = new ArrayList<>();
        RecycleRefrigertorManageAdapter adapter;
        ...
        public void requestData(String start, String end, String deviceId) {
            showProgressDialog();
            HttpGetController.getInstance().getFridgetorManageList(className);
        }

        public void handleData(String data) {
            Temps = new Gson().fromJson(data, RefrigeratorBean.class);
            ehmList.clear();
        ...
        public void requestAdd(String key,String value) {
            showProgressDialog();
            HttpGetController.getInstance().addFrageratorManage(className,key,value);
        }

        public void requestEdt(String key,String value) {
            showProgressDialog();
            HttpGetController.getInstance().edtFridgetorManage(className,key,value);
        }

        //
        public void requestDel(String id, String ip) {
            HttpGetController.getInstance().delFridgertorManage(id, ip, className);
        }
        ...
        public void showEdtDialog(final int position) {
            View body = LayoutInflater.from(getActivity()).inflate(R.layout.layout_fridgerator_input, null);
            ...
            String title = "设备修改";
            if (position != -1) {
                List<RefrigeratorBean.EhmListBean> ehmList = Temps.getEhmList();
            ...
            AlertDialog dialog = new AlertDialog.Builder(getActivity()).setTitle(title).setPositiveButton("提交", new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int which) {
                    ...
                    if (position == -1) {
                        ...
                        RefrigeratorBean.EhmListBean ehmListBean=new RefrigeratorBean.EhmListBean();
                       ...
                       /* ColdAlart.LocationListBean locationListBean=new ColdAlart.LocationListBean();
                        ...
                        requestAdd("IceboxManagePojo",EhmListBeanStr);
                        ...
                        requestEdt("IceboxManagePojo",EhmListBeanStr);
                    }
                }
