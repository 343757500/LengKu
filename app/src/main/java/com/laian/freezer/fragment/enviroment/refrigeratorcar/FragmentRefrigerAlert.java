...
import com.laian.freezer.adapter.RecycleRefrigerManageAdapter;
import com.laian.freezer.adapter.RecycleTempAlertAdapter;
import com.laian.freezer.adapter.RecycleTempManageAdapter;
...
public class FragmentRefrigerAlert extends FragmentAlert {
    ...
    public static class FragmentRefrigeManage extends FragmentAlert implements RecycleRefrigerManageAdapter.OnItemClickListner {
        String action_getData = className + API.getRefiiferTempManage;
        String action_add = className + API.addReFridge;
        String action_edt = className + API.edtReFridge;
        String action_del = className + API.delReFridge;
        String action_getIP = className + API.getTempIP;
        String action_getLocation = className + API.getTemplocations;
        ArrayList<RefrigerReal> Temps = new ArrayList<>();
        ArrayList<Location> locations = new ArrayList<>();
        ArrayList<Ip> ips = new ArrayList<>();
        RecycleRefrigerManageAdapter adapter;
        ...
        public void requestData(String start, String end, String deviceId) {
            showProgressDialog();
            HttpGetController.getInstance().getReFridgeManageList(className);
        }

        public void handleData(String data) {
            ArrayList<RefrigerReal> temps = new Gson().fromJson(data, new TypeToken<ArrayList<RefrigerReal>>() {
            }.getType());
            Temps.clear();
            Temps.addAll(temps);
            adapter.notifyDataSetChanged();
        ...
        public void onItemDel(final int position) {
            AlertDialog alertDialog = new AlertDialog.Builder(getActivity()).setMessage("是否要删除" + Temps.get(position).getDevName() + "?").setNegativeButton("取消", new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int which) {
